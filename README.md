# Desenvolvimento de Sistemas - SENAI

Meu nome é Eduardo, com nickname de EdxzyLuksz e no momento em que escrevo esta descrição, tenho 16 anos e estou cursando Desenvolvimentos de Sistemas no SENAI. Este repositório será dedicado à toda atividade realizada no período do curso, desde apresentações em slides dos docentes, slideshows autorais, códigos, trabalhos escritos, práticas e até mesmo os certificados e palestras que, apesar de não serem diretamente relacionados ao curso, se tratam de obrigações impostas na grade.

Exceções (Certificados não obrigatórios):

- Python com Framework (2025)
- Inteligências Artificiais Generativas Aplicada a Programação - Chatgpt (2026)

## 2025

### 1o Termo - Disciplinas

- Wellington Serafim dos Santos Silva: Sistemas Operacionais / Levantamento de Requisitos
- William César Augustonelli: Arquitetura de Redes com IoT / Lógica de Programação e Algoritmos

### 2o Termo - Disciplinas

- Wellington Serafim dos Santos Silva: Programação Backend / Projeto de Software
- William César Augustonelli: Banco de Dados / Linguagens de Marcação

### Três últimos dia de aula

- César Perez Brandão: Criação de Jogos com Unity e C#
- Diogo Takamori Barbosa: Revisão Git/GitHub

Para 2026, os professores Wellington e William não estarão mais presentes na instituição para proporcionar novas aulas.

## 2026

### 3o Termo - Disciplinas

- Diogo Takamori Barbosa: Programação para Dispositivos Móveis / Programação Front-end ;
- Jorge Carneiro: Programação Back-end / Projeto de Software 2

## Softwares Utilizados

Pasta no Google Drive para a instalação dos Setups, Isos e Portáteis utilizados ao decorrer do curso:

- [Google Drive - Softwares](https://drive.google.com/drive/folders/1650XkWGCc-5uLBJ8kgAfvQyAYtIYnJJW?usp=drive_link)
